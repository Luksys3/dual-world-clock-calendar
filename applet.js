/*
 *   World Clock Calendar applet calendar@luksys3
...
const EXTENSION_UUID = "calendar@luksys3";
...
MyApplet.prototype = {
	...
	_init: function (orientation, panel_height, instance_id) {
		...
		try {
			...
			let addWorldClocks = Lang.bind(this, function () {
				if (this._worldclocks_box) {
					this._worldclocks_box.destroy();
				}
				...
				for (i in this._worldclocks) {
					this._worldclocks[i][1] = GLib.TimeZone.new(
						this._worldclocks[i][1]
					);

					let tz = new St.BoxLayout({ vertical: false });
					let tz_label = new St.Label({
						style_class: "calendar",
						text: _(this._worldclocks[i][0]),
					});
					tz.add(tz_label, {
					...
					this._worldclock_labels[i] = new St.Label({
						style_class: "calendar",
					});
					tz.add(this._worldclock_labels[i], {
	...
	_updateClockAndDate: function () {
		...
		if (!label_string) {
			...
			label_string =
				"~CLOCK FORMAT ERROR~ " +
				displayDate.toLocaleFormat(DEFAULT_FORMAT);
		}

		let displayDate2 = GLib.DateTime.new_now_utc().add_hours(-4);
		let label_string2 = displayDate2.format(this._dateFormat);
		...
		if (!label_string2) {
			...
			label_string2 =
				"~CLOCK FORMAT ERROR~ " +
				displayDate2.toLocaleFormat(DEFAULT_FORMAT);
		}
		...
		for (var i in this._worldclocks) {
			...
			tooltip.push(
				rpad(_(this._worldclocks[i][0]), "\xA0", this.max_length + 10) +
					tz
			);
		}
	...
	_updateClockAndDatePeriodic: function () {
		...
		this._periodicTimeoutId = Mainloop.timeout_add_seconds(
			1,
			Lang.bind(this, this._updateClockAndDatePeriodic)
		);
	},
	...
	_launch_worldclocks_config: function () {
		Util.spawnCommandLine(
			"/usr/bin/env python3 " +
				APPLET_DIR +
				"/world_clock_calendar_settings.py --instance-id " +
				this.instance_id
		);
	},
	...
	_initRightClickMenu: function () {
		...
		this._applet_context_menu.addMenuItem(
			new Applet.MenuItem(
				_("Edit World Clocks"),
				"system-run-symbolic",
				Lang.bind(this, this._launch_worldclocks_config)
			)
		);
		this._applet_context_menu.addMenuItem(
			new Applet.MenuItem(
				...
				"system-run-symbolic",
				Lang.bind(this, this._launch_dateandtime_settings)
			)
		);
	},
};
